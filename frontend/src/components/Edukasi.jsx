...
const ISPU_CATEGORIES = [
  {
    status: 'Baik', range: '0 – 50', icon: 'ti-mood-smile',
    bg: '#DCFCE7', border: '#86EFAC', textColor: '#14532D', iconColor: '#16A34A',
    desc: 'Kualitas udara memuaskan, tidak menimbulkan risiko kesehatan bagi siapapun.',
    activity: 'Semua aktivitas luar ruangan aman dilakukan.',
  },
  {
    status: 'Sedang', range: '51 – 100', icon: 'ti-mood-neutral',
    bg: '#FEF9C3', border: '#FDE047', textColor: '#713F12', iconColor: '#CA8A04',
    desc: 'Kualitas udara dapat diterima, namun beberapa polutan mungkin mempengaruhi kelompok sensitif.',
    activity: 'Kelompok sensitif sebaiknya mengurangi aktivitas berat di luar.',
  },
  {
    status: 'Tidak Sehat', range: '101 – 200', icon: 'ti-mood-sad',
    bg: '#FEF3C7', border: '#FCD34D', textColor: '#78350F', iconColor: '#D97706',
    desc: 'Semua orang mungkin mulai merasakan dampak kesehatan. Kelompok sensitif lebih terpengaruh.',
    activity: 'Kurangi aktivitas berat di luar, gunakan masker.',
  },
  {
    status: 'Sangat Tidak Sehat', range: '201 – 300', icon: 'ti-mood-sick',
    bg: '#FEE2E2', border: '#FECACA', textColor: '#7F1D1D', iconColor: '#DC2626',
    desc: 'Peringatan kesehatan darurat. Semua orang berisiko mengalami dampak serius.',
    activity: 'Hindari semua aktivitas luar ruangan. Gunakan masker N95.',
  },
  {
    status: 'Berbahaya', range: '> 300', icon: 'ti-skull',
    bg: '#1E1B4B', border: '#4338CA', textColor: '#E0E7FF', iconColor: '#818CF8',
    desc: 'Kondisi darurat kesehatan. Seluruh populasi kemungkinan terpengaruh serius.',
    activity: 'Tetap di dalam ruangan, tutup semua ventilasi, gunakan air purifier.',
  },
];
...
function Edukasi() {
  ...
  return (
    <div className="tab-content" style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
      ...
      {/* ── HERO SECTION ── */}
      <section style={{
        background: 'linear-gradient(145deg, #1C1B4B 0%, #2D2B8C 55%, #4B48CA 100%)',
        borderRadius: 24, padding: '36px', position: 'relative', overflow: 'hidden', marginBottom: 28,
      }}>
        <div style={{ position: 'absolute', top: -50, right: -50, width: 220, height: 220, borderRadius: '50%', background: 'rgba(139,92,246,0.18)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -30, left: -30, width: 160, height: 160, borderRadius: '50%', background: 'rgba(99,102,241,0.12)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ fontSize: 14, color: '#A78BFA', lineHeight: 1.7, marginBottom: 24, maxWidth: 540 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {[
              { icon: 'ti-flask',        label: '5 Polutan',           value: 'Dijelaskan',  color: '#C4B5FD' },
              { icon: 'ti-chart-bar',    label: '5 Kategori ISPU',     value: 'Lengkap',     color: '#34D399' },
              { icon: 'ti-shield-check', label: '6 Tips Perlindungan', value: 'Praktis',     color: '#60A5FA' },
              { icon: 'ti-bulb',         label: '4 Fakta Global',      value: 'Berdasar WHO',color: '#FCD34D' },
            ].map(item => (
              ...
              >
                ...
                <div style={{ fontSize: 10, fontWeight: 700, color: '#6D5BBB', marginBottom: 3, letterSpacing: '0.06em' }}>{item.label.toUpperCase()}</div>
      ...
      {activeTab === 'polutan' && (
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {POLLUTANTS.map(p => (
              <div key={p.name} style={{ background: p.bg, border: `1.5px solid ${p.border}`, borderRadius: 18, padding: '22px 24px', ...hoverCard }}
        ...
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {ISPU_CATEGORIES.map(cat => (
              <div key={cat.status} style={{ background: cat.bg, border: `1.5px solid ${cat.border}`, borderRadius: 18, padding: '20px 24px', display: 'flex', alignItems: 'flex-start', gap: 18, ...hoverCard }}
        ...
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
            {DAMPAK_KESEHATAN.map(item => (
              <div key={item.organ} style={{ background: item.bg, border: `1.5px solid ${item.border}`, borderRadius: 18, padding: '20px 22px', ...hoverCard }}
      ...
      {activeTab === 'tips' && (
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
            {TIPS.map(tip => (
              <div key={tip.title} style={{ background: tip.bg, border: `1.5px solid ${tip.border}`, borderRadius: 18, padding: '20px 22px', display: 'flex', gap: 16, alignItems: 'flex-start', ...hoverCard }}
        ...
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14, marginBottom: 16 }}>
            {FAKTA_MENARIK.map(fakta => (
              <div key={fakta.label} style={{ background: fakta.bg, border: `1.5px solid ${fakta.border}`, borderRadius: 18, padding: '22px', ...hoverCard }}
      ...
      {activeTab === 'panduan' && (
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {INFO_AERIS.map(info => (
              <div key={info.title} style={{ background: info.bg, border: `1.5px solid ${info.border}`, borderRadius: 18, padding: '20px 24px', display: 'flex', gap: 18, alignItems: 'flex-start', ...hoverCard }}
        ...
        <section style={{ marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginBottom: 16 }}>
            {KELOMPOK_SENSITIF.map(k => (
              <div key={k.label} style={{ background: k.bg, border: `1.5px solid ${k.border}`, borderRadius: 16, padding: '18px 20px', display: 'flex', gap: 14, alignItems: 'center', ...hoverCard }}
      ...
      {/* ── FOOTER CTA ── */}
      <section style={{
        background: 'linear-gradient(145deg, #1C1B4B 0%, #2D2B8C 60%, #4B48CA 100%)',
        borderRadius: 20, padding: '28px 32px', textAlign: 'center',
        position: 'relative', overflow: 'hidden', marginTop: 20,
      }}>
        <div style={{ position: 'absolute', top: -30, right: -30, width: 160, height: 160, borderRadius: '50%', background: 'rgba(139,92,246,0.18)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            {[
              { icon: 'ti-home',           label: 'Kembali ke Beranda', color: '#C4B5FD' },
              { icon: 'ti-crystal-ball',   label: 'Lihat Prediksi',     color: '#34D399' },
